...
from config import ADDRESSES_PER_PAGE, BITCOIN_MAX_NUMBER, FLASK_HOST, FLASK_PORT, FLASK_DEBUG, MAX_SEARCH_PAGES
...
def home_page():
    ...
    for item in items:
        ...
        all_addresses.append(item.address_p2sh)
    
    watchlist_matches = watchlist_service.find_matching_addresses(all_addresses)
    
    # Add watchlist match indicator to items
    for item in items:
        item.is_watchlist_match_compressed = item.address_compressed in watchlist_matches
        item.is_watchlist_match_uncompressed = item.address_uncompressed in watchlist_matches
        item.is_watchlist_match_p2sh = item.address_p2sh in watchlist_matches
...
@app.route('/random')
def random_page():
    """Redirect to a random page with uniform distribution across percentage range"""
    import random
    import math
    ...
    # Pick a random percentage (0-100) instead of random page number
    # This gives uniform distribution across the entire percentage range
    random_percentage = random.uniform(0, 100)
    
    # Convert percentage back to page number using logarithmic scale
    log_max = math.log10(max_page)
    log_page = (random_percentage / 100) * log_max
    random_page_num = int(10 ** log_page)
    
    # Ensure it's within valid range
    random_page_num = max(1, min(random_page_num, max_page))
