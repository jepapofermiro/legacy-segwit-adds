...
class AllKeyService:
    ...
    def _get_p2sh_address(self, key_hex: str) -> str:
        """Generate SegWit P2SH (Pay-to-Script-Hash) address"""
        try:
            # Convert hex to bytes
            private_key_bytes = bytes.fromhex(key_hex)
            ...
            # Create witness program (OP_0 + push(20 bytes of pubkey hash))
            pubkey_hash = hashlib.new('ripemd160', hashlib.sha256(public_key_compressed).digest()).digest()
            witness_program = b'\x00\x14' + pubkey_hash  # 0x00 = OP_0, 0x14 = push 20 bytes
            
            # Hash the witness program to create the script hash
